...
export async function compileAeternityBridge(sdk: AeSdk): Promise<ContractWithMethods<ContractMethodsBase>> {
    ...
    // UPDATE ABI
    fs.writeFileSync(`${__dirname}/../__SNAPSHOT__/bridge.aci`, JSON.stringify(bridge._aci), { encoding: "utf-8" });
    return bridge;
}
...
export async function deployAeternityBridge(sdk: AeSdk, assets: { [symbol:string] : { aeternity: string, ethereum: string }}): Promise<string> {
    ...
    const bridge = await compileAeternityBridge(sdk);
...
export async function updateProcessors(sdk: AeSdk, bridgeAddress: string): Promise<void> {
    ...
    for (let processor of processors.aeternity) {
        (await bridge.add_signer(processor));
    }
}
